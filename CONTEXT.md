...
- **Stream**: The internal transport mechanism used to synchronize a **Collection**. Avoid using Stream as the public DSL name because it is overloaded with LiveView socket transport and Phoenix LiveStream.
...
- **Stream Query**: A page-level binding that applies a paginated or filtered command result to an active **Route Collection** using server-side Phoenix stream operations such as append, prepend, reset, and limit. Vue does not manually reconcile collection-owned data.
...
- **Collection Block**: The resource-level DSL boundary that declares synchronization for a named server-owned reactive list. A **Collection Block** must declare an explicit source event for initial/reset data; insert/update/delete mappings are optional, but a source-only Collection should warn because it will not update from PubSub. It uses a domain-unique collection name and is distinct from command orchestration and from a **Subscribe Block**, which delivers semantic callback events to Vue.
