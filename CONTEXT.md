...
- **Exposed Name**: The client-facing string name that crosses the Vue boundary. Exposed event names and exposed signal names are application-wide unique within a host app so generated SDK calls and `ash.on()` listeners do not depend on page-scoped Domain lookup.
...
- **Alva Client API**: A suite of custom Vue-side wrappers and composables that provide a highly advanced, type-safe API over LiveVue. It includes:
  ...
  - `usePageEvent`: For executing page-local UI orchestration events. These rely on route-specific codegen rather than the global registry to avoid type collisions across LiveViews.
