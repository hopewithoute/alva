defmodule Mix.Tasks.Alva.Codegen do
  ...
  defp generate_subscriptions_ts(subscription_map, events_map, output_dir) do
    subs_content =
      subscription_map
      |> Enum.map(fn {key, {resource, sub_def}} ->
        kind = sub_def.kind

        input_shape =
          if sub_def.scope && sub_def.scope != %{} do
            # Extremely basic scope mapping for now
            fields = Enum.map(sub_def.scope, fn {k, v} ->
              ts_type = Alva.Codegen.TypeMapper.map_type(v)
              "#{k}: #{ts_type};"
            end)
            "{ #{Enum.join(fields, " ")} }"
          else
        ...
        payload_prop =
          if kind == :stream do
            if sub_def.source do
              ...
              {_, event_def} = Enum.find(Map.values(events_map), fn {_, ev} -> ev.key == source_event_key end)
              output_type = Alva.Codegen.DtoGenerator.output_type(resource, event_def) |> String.replace_suffix("[]", "")
