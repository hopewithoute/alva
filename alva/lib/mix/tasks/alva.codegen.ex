defmodule Mix.Tasks.Alva.Codegen do
  ...
  def run(_args) do
    ...
    if Ash.Info.domains(app) == [] do
      ...
      generate_use_alva_ts(events_map, signal_map, composables_dir)
      generate_use_alva_form_ts(composables_dir)
      generate_use_alva_query_ts(composables_dir)
      generate_use_alva_paginated_query_ts(composables_dir)
      generate_use_alva_upload_ts(composables_dir)
      generate_use_alva_assigns_ts(composables_dir)
      generate_testing_utils_ts(events_map, output_dir)
  ...
  defp generate_use_alva_ts(events_map, signal_map, composables_dir) do
    all_keys = Map.keys(events_map) ++ Map.keys(signal_map)
    ...
    write_file!(composables_dir, "useAlva.ts", content)
  end
  ...
  defp generate_use_alva_form_ts(composables_dir) do
    ...
    >(
        ...
        const liveForm = useLiveForm(pseudoForm, {
            ...
            changeEvent: (options.validateEvent as unknown as string) || null,
        ...
        const submit = async (): Promise<AlvaEvents[K]["output"]> => {
            for (const key of Object.keys(pseudoForm.values as object)) {
                try {
                    const fieldRef = liveForm.field(key as any);
                    if (fieldRef && fieldRef.value && fieldRef.value.value !== undefined) {
                        (pseudoForm.values as any)[key] = fieldRef.value.value;
                    }
                } catch (_e) {}
            }
  ...
  defp generate_use_alva_query_ts(composables_dir) do
    content = """
    // This file is auto-generated by Alva.
    // Do not edit manually.
    ...
    write_file!(composables_dir, "useAlvaQuery.ts", content)
  end
  ...
  defp generate_use_alva_assigns_ts(composables_dir) do
    content = """
    // This file is auto-generated by Alva.
    // Do not edit manually.
    ...
    write_file!(composables_dir, "useAlvaAssigns.ts", content)
  end

  defp generate_use_alva_paginated_query_ts(composables_dir) do
    content = """
    // This file is auto-generated by Alva.
    // Do not edit manually.
    ...
    write_file!(composables_dir, "useAlvaPaginatedQuery.ts", content)
  end

  defp generate_testing_utils_ts(_events_map, output_dir) do
    testing_dir = Path.join(output_dir, "testing")
    File.mkdir_p!(testing_dir)
    ...
    write_file!(testing_dir, "index.ts", content)
  end
  ...
  defp generate_domain_events(events_map, domain, domain_prefix) do
    ...
    |> Enum.map_join(",\n", fn {event_name, {resource, event_def}} ->
      ...
      lookup_field = Map.get(event_def, :lookup) || :id

      by_id_ts =
        if is_read do
          """
          ,
                  use_#{action_name}_by_#{lookup_field}: (
                      ...
                      const inputGetter = () => {
                          ...
                          return id ? ({ #{lookup_field}: id } as any) : null;
                      };
      ...
      """
              #{action_name}: (payload: AlvaEvents["#{event_name}"]["input"]): Promise<AlvaEvents["#{event_name}"]["output"]> => {
                  return new Promise((resolve) => {
                      ...
                      safePushEvent("#{event_name}", payload, (reply: AlvaEvents["#{event_name}"]["output"]) => {
                          const durationMs = Math.round(performance.now() - startTime);
                          if (typeof window !== "undefined" && (window as any).__ALVA_RECORD_TELEMETRY__) {
                              (window as any).__ALVA_RECORD_TELEMETRY__("#{event_name}", durationMs, reply?.ok);
                          }
                          if (reply?.ok) {
              ...
              use_#{action_name}_upload: (options?: import("./useAlvaUpload").AlvaUploadOptions) => {
                  return useAlvaUpload("#{event_name}", options);
              }#{query_ts}
      """
    end)
  end
    ...
    |> Enum.map_join(",\n", fn {_k, {_, sig}} ->
      ...
      """
              on_#{signal_name}: (
                  ...
                  if (live) live.pushEvent("alva:subscribe_signal", { name: "#{sig.name}", input }, () => {});
                  useLiveEvent("#{sig.name}", callback);
                  onUnmounted(() => {
                      if (live) live.pushEvent("alva:unsubscribe_signal", { name: "#{sig.name}", input }, () => {});
                  });
              },
              use_#{signal_name}_state: (
                  input: AlvaSignals["#{sig.name}"] extends { input: infer I } ? I : Record<string, never> = {} as any
              ) => {
                  const data = ref<AlvaSignals["#{sig.name}"]["payload"][]>([]);
                  const latest = ref<AlvaSignals["#{sig.name}"]["payload"] | null>(null);
                  const count = computed(() => data.value.length);
                  const clear = () => { data.value = []; latest.value = null; };

                  if (live) live.pushEvent("alva:subscribe_signal", { name: "#{sig.name}", input }, () => {});
                  useLiveEvent("#{sig.name}", (payload: any) => {
