defmodule Alva.ErrorTest do
  ...
  test "formats not found from Invalid wrapping NotFound" do
    invalid = %Ash.Error.Invalid{
      errors: [%Ash.Error.Query.NotFound{primary_key: %{id: 1}, resource: User}]
    }
  ...
  test "formats validation error with field and bread_crumbs" do
    ...
    assert %{
             type: "validation",
             fields: %{"name" => ["Invalid value provided for name: is required."]}
  ...
  test "formats validation error with path" do
    ...
    assert %{
             type: "validation",
             fields: %{"user.profile.name" => ["Invalid value provided for name: is invalid."]}
           } = Error.format(invalid)
  end
  ...
  test "formats validation error with path ending in field" do
    ...
    assert %{
             type: "validation",
             fields: %{"user.name" => ["Invalid value provided for name: is invalid."]}
           } = Error.format(invalid)
  end
  ...
  test "formats unknown error when exposed" do
    ...
    assert %{type: "unknown", message: "an error"} =
             Error.format(%RuntimeError{message: "an error"}, [])

    if old,
      do: Application.put_env(:alva, :expose_unknown_errors, old),
      else: Application.delete_env(:alva, :expose_unknown_errors)
  end
  ...
  test "formats unknown error when not exposed" do
    ...
    assert %{type: "unknown", message: "An unexpected error occurred"} =
             Error.format(%RuntimeError{message: "an error"})
