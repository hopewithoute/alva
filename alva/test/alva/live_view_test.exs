defmodule Alva.LiveViewTest do
  ...
  defmodule TestResource do
    ...
    live_vue do
      ...
      subscription :test_signal, name: "test_signal", resolve: :resolve_signal, kind: :signal, on: :create
  ...
  describe "on_mount/4" do
    ...
    test "configures file uploads from commands", %{socket: socket} do
      config = %{subscriptions: [], commands: [:upload_command, :unknown_command, :unknown_action_command]}
      {:cont, socket} = Alva.LiveView.on_mount(config, %{}, %{}, socket)
  ...
  describe "handle_event hooks" do
    ...
    test "alva:deactivate_subscription", %{socket: socket, handle_event: handle_event} do
      # first activate
      {:halt, _, socket} = handle_event.("alva:activate_subscription", %{"name" => "test_stream"}, socket)
    ...
    test "alva:activate_subscription success", %{socket: socket, handle_event: handle_event} do
      params = %{"name" => "test_stream", "input" => %{}}
      assert {:halt, %{ok: true, data: _}, _socket} = handle_event.("alva:activate_subscription", params, socket)
    end
    
    test "alva:load_more_subscription success", %{socket: socket, handle_event: handle_event} do
      params = %{"name" => "test_stream", "input" => %{}}
      assert {:halt, %{ok: true, data: _}, _socket} = handle_event.("alva:load_more_subscription", params, socket)
    end
    
    test "alva:load_more_subscription failure", %{socket: socket, handle_event: handle_event} do
      params = %{"name" => "unknown_stream", "input" => %{}}
      assert {:halt, %{ok: false, error: %{type: "not_found"}}, _socket} = handle_event.("alva:load_more_subscription", params, socket)
    end
    
    test "alva:load_more_subscription forbidden", %{socket: socket, handle_event: handle_event} do
      params = %{"name" => "test_signal", "input" => %{}}
      assert {:halt, %{ok: false, error: %{type: "forbidden"}}, _socket} = handle_event.("alva:load_more_subscription", params, socket)
    end
    
    test "alva:load_more_subscription error", %{socket: socket, handle_event: handle_event} do
      params = %{"name" => "test_error", "input" => %{}}
      assert {:halt, %{ok: false, error: _}, _socket} = handle_event.("alva:load_more_subscription", params, socket)
    end
    
    test "alva:deactivate_subscription success", %{socket: socket, handle_event: handle_event} do
      # First activate
      {:halt, _, socket} = handle_event.("alva:activate_subscription", %{"name" => "test_stream", "input" => %{}}, socket)
      
      # Activate again to get count > 1
      {:halt, _, socket} = handle_event.("alva:activate_subscription", %{"name" => "test_stream", "input" => %{}}, socket)
      
      # Deactivate once
      assert {:halt, %{ok: true}, socket} = handle_event.("alva:deactivate_subscription", %{"name" => "test_stream"}, socket)
      
      # Deactivate again
      assert {:halt, %{ok: true}, socket} = handle_event.("alva:deactivate_subscription", %{"name" => "test_stream"}, socket)
      
      # Deactivate when missing to hit _ -> refs
      assert {:halt, %{ok: true}, _socket} = handle_event.("alva:deactivate_subscription", %{"name" => "test_stream"}, socket)
    end
    
    test "alva:deactivate_subscription failure", %{socket: socket, handle_event: handle_event} do
      assert {:halt, %{ok: false}, _socket} = handle_event.("alva:deactivate_subscription", %{"name" => "unknown_stream"}, socket)
    end
    ...
    test "dispatches unknown events to Alva.Dispatcher", %{socket: socket, handle_event: handle_event} do
  ...
  describe "handle_info hooks" do
    setup %{socket: socket} do
      ...
      hook_event = Enum.find(socket.private.lifecycle.handle_event, &(&1.id == :alva_handle_event))
      {:halt, _, socket} = hook_event.function.("alva:activate_subscription", %{"name" => "test_stream", "input" => %{}}, socket)
      {:halt, _, socket} = hook_event.function.("alva:activate_subscription", %{"name" => "test_signal", "input" => %{}}, socket)
