defmodule Alva.DispatcherUncoveredTest do
  ...
  defmodule ErrorResource do
    use Ash.Resource, domain: Alva.DispatcherUncoveredTest.ErrorDomain, extensions: [Alva.Resource], data_layer: Ash.DataLayer.Ets
  ...
  setup do
    ...
    try do :persistent_term.erase({{Alva.Registry, :registry}, :alva}) rescue _ -> :ok end
  ...
  test "unwrap param when event name is in payload" do
    assert %{ok: false, error: %{type: "conflict"}} = Alva.Dispatcher.dispatch("test_create", %{"test_create" => %{}}, otp_app: :alva)
  end
  
  test "create outer error" do
    assert %{ok: false, error: %{type: "conflict"}} = Alva.Dispatcher.dispatch("test_create", %{}, otp_app: :alva)
  end
  
  test "update outer error" do
    record = ErrorResource |> Ash.Changeset.for_create(:create_ok) |> Ash.create!()
    assert %{ok: false, error: %{type: "conflict"}} = Alva.Dispatcher.dispatch("test_update", %{"id" => record.id}, otp_app: :alva)
  end
  
  test "update inner error" do
    assert %{ok: false, error: %{type: "not_found"}} = Alva.Dispatcher.dispatch("test_update", %{"id" => Ash.UUID.generate()}, otp_app: :alva)
  end
  
  test "destroy inner error" do
    record = ErrorResource |> Ash.Changeset.for_create(:create_ok) |> Ash.create!()
    assert %{ok: false, error: %{type: "conflict"}} = Alva.Dispatcher.dispatch("test_destroy", %{"id" => record.id}, otp_app: :alva)
  end
  
  test "destroy ok returns record" do
    record = ErrorResource |> Ash.Changeset.for_create(:create_ok) |> Ash.create!()
    assert %{ok: true} = Alva.Dispatcher.dispatch("test_destroy_ok", %{"id" => record.id}, otp_app: :alva)
  end
  
  test "read_one inner error" do
    assert %{ok: false, error: _} = Alva.Dispatcher.dispatch("test_read_one", %{"id" => Ash.UUID.generate()}, otp_app: :alva)
  end
  ...
  test "get returns not found" do
    assert %{ok: false, error: _} = Alva.Dispatcher.dispatch("test_get", %{"id" => Ash.UUID.generate()}, otp_app: :alva)
  end

  test "read lookup inner error" do
    assert %{ok: false, error: _} = Alva.Dispatcher.dispatch("test_read_lookup_err", %{"id" => Ash.UUID.generate()}, otp_app: :alva)
  end

  test "get empty returns not found" do
    assert %{ok: false, error: %{type: "not_found"}} = Alva.Dispatcher.dispatch("test_get_empty", %{"id" => Ash.UUID.generate()}, otp_app: :alva)
  end

  test "get returns ok" do
    assert %{ok: true} = Alva.Dispatcher.dispatch("test_get_ok", %{"id" => Ash.UUID.generate()}, otp_app: :alva)
  end

  test "action inner error" do
    assert %{ok: false, error: %{type: "unknown"}} = Alva.Dispatcher.dispatch("test_action", %{}, otp_app: :alva)
  end
  
  test "sort parse input invalid" do
    assert %{ok: false, error: _} = Alva.Dispatcher.dispatch("test_read", %{sort: "invalid_format"}, otp_app: :alva)
  end
  ...
  test "find_event_in_domains missing returns error" do
    assert %{ok: false, error: %{type: "unknown"}} = 
      Alva.Dispatcher.dispatch("non_existent_event", %{}, domains: [Alva.DispatcherUncoveredTest.ErrorDomain])
  end

  test "test_list_string" do
    assert %{ok: true, data: ["string"]} = Alva.Dispatcher.dispatch("test_list_string", %{}, otp_app: :alva)
  end
