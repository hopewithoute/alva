...
export interface AlvaUploadHandle {
    name: string;
...
export interface AlvaUploadOptions {
    maxFiles?: number;
    maxSize?: number; // in bytes
}

export interface AlvaUploadDispatchContext {
    files: UploadEntry[];
    references: string[];
    primaryReference: string;
}

export interface AlvaUploadDispatchOptions {
    clear?: boolean;
}
...
export function useAlvaUpload(name: string, options?: AlvaUploadOptions) {
    ...
    const upload = useLiveUpload(getUploadConfig, {
        ...
        ...(options || {})
    }) as unknown as AlvaUploadHandle;
    ...
    const waitForCompletion = () => {
        return new Promise<AlvaUploadDispatchContext>((resolve, reject) => {
            let settled = false;
    ...
    const dispatch = async <T>(
        submit: (upload: AlvaUploadDispatchContext) => Promise<T>,
        options: AlvaUploadDispatchOptions = {},
    ) => {
        try {
