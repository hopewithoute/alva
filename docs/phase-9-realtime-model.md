...
* **Route Subscription**: A page-level declaration indicating which PubSub topics a given LiveView page cares about (e.g., `Alva.LiveView.subscribe(socket, "orders:all")`). Route Subscriptions define scope only; they do not by themselves choose whether an incoming occurrence becomes a Collection update or a Signal callback.
