...
Based on ADR 0002, Alva's realtime communication model separates concerns into **Commands**, **Route Subscriptions**, **Streams**, **Route Collections**, **Stream Queries**, **Signals**, **Resource Projections**, and **Page Projections**.
...
* **Resource Projection**: The foundational block inside an Ash Resource (the Stream Block or Subscribe Block) dictating what happens when an Ash PubSub event fires.
...
defmodule MyAppWeb.HomeLive do
  ...
  def mount(_params, _session, socket) do
    ...
    %{ok: true, data: students} = Alva.Dispatcher.dispatch("students.list", %{}, domains: socket.private.alva.domains)
...
const refresh_list = async () => {
  // Calls the server; the result is caught by `bind_stream_query` and updates the stream
