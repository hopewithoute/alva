...
- [x] An incoming notification matching an active Signal projection pushes the semantic signal name to Vue.
- [x] Signal payloads use the established Projection Rule and do not expose raw Ash resource internals.
- [x] Notifications for inactive Signal projections are ignored by that page.
- [x] The same published occurrence can be used as both a Stream and a Signal when a page activates both.
- [x] The existing generic `ash_notification` behavior is replaced or quarantined so it is not the public Signal contract.
- [x] Tests cover async/non-collection Signal delivery without mutating a Route Collection.
