...
## Acceptance criteria
- [ ] Alva DSL supports declaring an event strictly for validation (e.g. `validate_only: true`).
- [ ] The `Alva.Dispatcher` executes the target Ash action securely as a dry-run (e.g., stopping after validation but before DB commit).
- [ ] Returns properly shaped `LiveResult` (`ok: true` if valid, `ok: false, error: LiveError` if invalid).
- [ ] Validated by ExUnit tests verifying no database side-effects occur.
