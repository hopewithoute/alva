...
## Acceptance criteria
- [ ] Embedded resources, Unions, and Enums generate accurate nested interfaces or union string literals.
- [ ] `enable_filter: true` causes the event input to accept a strictly typed `filter` property (based on the resource's fields).
- [ ] Ash Filter AST logic (e.g. `and`, `or`, `ilike`) is accurately exported to `types.ts`.
