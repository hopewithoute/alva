...
## Acceptance criteria
- [x] `ashForm.ts` reliably cancels pending debounce logic upon full submission or newer validations.
- [x] Vitest spec tests are added to `alva/assets/js/ashForm.spec.ts` mocking the `api.call` response and asserting correctness during race-condition scenarios.
- [x] No regression occurs in `ashForm`'s existing error application logic.
