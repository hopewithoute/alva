...
## Acceptance criteria
- [x] Incoming PubSub `update` notifications evaluate the updated record against the stream's `scope`.
- [x] If an updated record falls out of scope, it correctly triggers a `stream_delete` instead of a `stream_insert`.
